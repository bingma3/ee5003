...
def get_host_ip():  # find the device ip address
...
def seeding(path, save_dir, enable=True):
    """
    Seeding is based on P2P protocol to share the data between the nodes
    :param path: the path of torrent file
    :param save_dir: the destination of the data
    :param enable: enable/disable the seeding
    :return: True when the download reach 100%
    """
    ses = lt.session()  # start a session
    with open(path, 'rb') as f:  # read the torrent file
        torrent = lt.bdecode(f.read())
    params = {
        'save_path': save_dir,
        'storage_mode': lt.storage_mode_t(2),   # set the storage to sparse mode
...
def read_file(path):    # read the binary file
...
def update_content(client, ip, port, path, save):
    try:
        ...
        if client.send_request(ip, port):   # send the request to ask the new torrent file via MQTT
            logger.info(f"Seed updated")
            if seeding(path, save):     # start the data transmit via BT
                return True
        else:
...
def remove_content(dir):    # remove the file after download complete, prepare for the next cycle
    try:
        os.remove(dir)
    except Exception as e:
        shutil.rmtree(dir)
...
def run():  # test script
    ...
    cnt = 1000  # set up 1000 cycle

    mqtt_client = SmartOohClient(torrentfile_path, save_dir)    # connect to the MQTT broker
